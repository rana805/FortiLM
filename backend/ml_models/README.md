...
This directory contains custom trained ML models for FortiLM security modules.
...
import joblib
joblib.dump(model, 'backend/ml_models/pii_detector/pii_detector.joblib')

# Example: Save TensorFlow model
model.save('backend/ml_models/toxicity_detector/toxicity_detector.h5')

# Example: Save PyTorch model
torch.save(model.state_dict(), 'backend/ml_models/bias_detector/bias_detector.pt')
```

### 3. Model Auto-Loading

The modules will automatically try to load models on initialization. If a model is found, it will be used. Otherwise, the system falls back to rule-based detection.
...
```bash
# .env file
PII_MODEL_PATH=ml_models/pii_detector/custom_model.pkl
TOXICITY_MODEL_PATH=ml_models/toxicity_detector/custom_model.h5
BIAS_MODEL_PATH=ml_models/bias_detector/custom_model.pt
JAILBREAK_MODEL_PATH=ml_models/jailbreak_detector/custom_model.onnx
```
